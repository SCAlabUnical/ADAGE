...
public class WorkflowDatacenter extends Datacenter {
	...
	private static final double RANDOM = 100;

	public WorkflowDatacenter(String name, DatacenterCharacteristics characteristics, VmAllocationPolicy vmAllocationPolicy, List<Storage> storageList, double schedulingInterval) throws Exception {
	...
	public List<Storage> getStorageList() {
		...
		storage.addAll(super.getStorageList());
		for (Vm vm : getVmList()) {
			if (vm instanceof CondorVM) {
				CondorVM cvm = (CondorVM) vm;
				storage.addAll(cvm.getStorage());
			}
		}
		return storage;
	}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (job.isFinished()) {
				...
				Log.printLine(getName() + ": Warning - Cloudlet #" + job.getCloudletId() + " owned by " + name + " is already completed/finished.");
			...
			case DATACENTER:
				...
				job.setResourceParameter(getId(), getCharacteristics().getCostPerSecond(), getCharacteristics().getCostPerBw());
				break;
			case VM:
	...
	protected void processDataDelete(SimEvent ev, boolean ack) {
		for (Storage storage : getStorageList()) {
			LinkedList<FileItem> daRimuovere = (LinkedList<FileItem>) ev.getData();
			for (FileItem fileItem : daRimuovere) {
				if (storage.contains(fileItem.getName())) {
					storage.deleteFile(fileItem.getName());
				}
			}
		}
	}
...
//			double task_runtime = randomize(RANDOM, task.getCloudletLength() / vm.getMips());
			start_time += task_runtime;
			// Because CloudSim would not let us update end time here
			task.setTaskFinishTime(start_time);
			if (start_time>max)max=start_time;
        }
        job.setTaskFinishTime(max);
	}

	private double randomize(double d, double value) {
		return Math.max(value * (1 + d * (Math.random() - 0.5)), 0.1);
	}
		...
		for (FileItem file : requiredFiles) {
			...
			if (file.isRealInputFile(requiredFiles)) {
				...
				case LOCAL:
					...
					CondorVM cvm = (CondorVM) vm;
					ClusterStorage source = null;
					ClusterStorage localStorage = (ClusterStorage) cvm.getStorage().get(0);
...
//							double maxTranferRate = 0;													
//							for (Storage storage : getStorageList()) {
//								maxTranferRate = 0;
//								if (storage.contains(file.getName())) {
//									if (storage.getMaxTransferRate() > maxTranferRate) {
//										maxTranferRate = storage.getMaxTransferRate();
//									}
//								}
//								}
//							if (maxTranferRate > 0)
//								time += file.getSize() / (double) Consts.MILLION / maxTranferRate;		
						...
						if (site.equals(Parameters.SOURCE)) {
							...
							bwth = localStorage.getMaxBandwidth("source");
							// bwth = dcStorage.getBaseBandwidth();
						} else {
							// transfers between two VMs is limited to both VMs
							source = (ClusterStorage) ((CondorVM) getVmList().get(Integer.parseInt(site))).getStorage().get(0);
							bwth = Math.min(localStorage.getMaxBandwidth("local"), source.getMaxBandwidth("local"));
						}
						if (bwth > maxBwth) {
							maxBwth = bwth;
						}
					...
					if (cvm.getStorage().get(1).getAvailableSpace() > file.getSize() && ReplicaCatalog.isPriority(file.getName(), getVmList().get(0).getId())) {
						cvm.getStorage().get(1).addFile(new File(file.getName(), (int) file.getSize()));
					} else {
						cvm.getStorage().get(0).addFile(new File(file.getName(), (int) file.getSize()));
					}
