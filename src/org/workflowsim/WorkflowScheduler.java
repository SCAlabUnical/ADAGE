...
public class WorkflowScheduler extends DatacenterBroker {
	...
	private BaseSchedulingAlgorithm algorithm;
	/**
	...
	private BaseSchedulingAlgorithm getScheduler(SchedulingAlgorithm name) {
		...
		case ROUNDROBIN:
			...
			break;
		case WORKGIVING:
			algorithm = new WorkGivingSchedulingAlgorithm();
			break;
		default:
			algorithm = new StaticSchedulingAlgorithm();
			break;
	...
	protected void processVmCreate(SimEvent ev) {
		...
		if (result == CloudSimTags.TRUE) {
			...
			if (VmList.getById(getVmList(), vmId) != null) {
				...
				Log.printLine(CloudSim.clock() + ": " + getName() + ": VM #" + vmId + " has been created in Datacenter #" + datacenterId + ", Host #"
...
//        CondorVM vm = (CondorVM) getVmsCreatedList().get(cloudlet.getVmId());
	...
	private void deleteOldFile(Job job) {
		LinkedList<FileItem> daRimuovere = new LinkedList<>();
		for (FileItem file : job.getFileList()) {
			daRimuovere.add(file);
			for (Cloudlet cloudlet : getCloudletList()) {
				Job j = (Job) cloudlet;
				if (j.getFileList().contains(file)) {
					daRimuovere.remove(file);
					break;
				}
			}
		}
		sendNow(getDatacenterIdsList().get(0), CloudSimTags.VM_DATA_DEL, daRimuovere);
